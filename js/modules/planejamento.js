window.planejamentoModule = {
  ...
  processar({ mes, ano, mesesData, gastosAno, contasPagar, contasReceber }) {
    ...
    const planejamentoMeses = this.montarPlanejamentoMeses({
      ano,
      mesesData,
      gastosAno
    });
  ...
  montarPlanejamentoMeses({ ano, mesesData, gastosAno }) {
    return this.meses.map(mes => {
      const registro = mesesData.find(item =>
        this.normalizarMes(item.mes || item.nome_mes || item.mes_ref) === mes
      ) || {};

      const faturado = this.numero(registro.faturado || registro.faturamento || 0);
      const aFaturar = this.numero(registro.a_faturar || 0);
      const receita = faturado + aFaturar;
  ...
  renderTabelaMeses(a) {
    ...
    const linhas = a.planejamentoMeses.filter(item =>
